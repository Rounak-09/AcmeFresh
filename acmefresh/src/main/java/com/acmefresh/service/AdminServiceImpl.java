...
public class AdminServiceImpl implements AdminService {
	...
	public Admin saveAdmin(Admin admin) throws AdminException {
		...
		Admin existingAdmin = adminDao.findByPhone(phone);
		if(String.valueOf(phone).length()==10) {
			if(existingAdmin==null) {
				return adminDao.save(admin);
			}
			else {
				throw new AdminException("Admin already exists with phone number : "+phone);
			}
		}
		else {
	...
	public AdminSession saveLogin(Admin admin) throws AdminException {
		...
		if(existingAdmin==null) {
			...
			if(session.isPresent()) {
				throw new AdminException("User already logged in");
			}
